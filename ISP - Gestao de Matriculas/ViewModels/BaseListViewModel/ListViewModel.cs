...
namespace ISP.GestaoMatriculas.BaseListViewModel
{
    ...
    public class ListViewModel : IListViewModel 
    {
        ...
        public void SetParameters(int tabNumber, int currentPageNumber, string sortColumn, string sortDirection, NameValueCollection requestParameters)
        {
            TabNumber = tabNumber;
        ...
        public void SetFilters(List<ListViewFilter> filters)
        {
            ...
            foreach (var filter in filters)
            {
                ...
                if(property != null)
                {
                    ...
                    {
                        Type typeProperty = property.PropertyType;
                        if (typeProperty.FullName == "System.Boolean" && filter.Value.ToString().Contains(','))
                        {
                            filter.Value = filter.Value.ToString().Substring(0, filter.Value.ToString().IndexOf(','));
                        }
                        if (typeProperty.FullName.Contains("System.Nullable") && filter.Value.ToString() == string.Empty)
                        {
                            continue;
                        }
                        castedObject = Convert.ChangeType(filter.Value, typeProperty);
                    }
                    //object value = (typeProperty)filter.Value;
                    property.SetValue(this, castedObject, null);
                }
            }
        }
